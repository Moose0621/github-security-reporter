...
EOF
    ...
    if [ "$SECRET_SCANNING_COUNT" -gt 0 ]; then
        echo "$SECRET_SCANNING_ALERTS" | jq -r '.[] | @json' | while IFS= read -r alert; do
            SECRET_TYPE=$(echo "$alert" | jq -r '.secret_type')
            SECRET_STATE=$(echo "$alert" | jq -r '.state')
            SECRET_LOCATION=$(echo "$alert" | jq -r '.locations[0].details.path // "N/A"')
            ...
            cat >> "$OUTPUT_DIR/summary.html" << EOF
                <div class="alert-item high">
                    ...
                    <div class="alert-location">$SECRET_LOCATION</div>
                </div>
EOF
        done
    else
        echo '                <div class="no-data">✅ No secret scanning alerts found</div>' >> "$OUTPUT_DIR/summary.html"
    fi
...
EOF
    ...
    if [ "$DEPENDABOT_COUNT" -gt 0 ]; then
        echo "$DEPENDABOT_ALERTS" | jq -r '.[] | @json' | while IFS= read -r alert; do
            DEPENDABOT_PACKAGE=$(echo "$alert" | jq -r '.security_advisory.package.name')
            DEPENDABOT_SEVERITY=$(echo "$alert" | jq -r '.security_advisory.severity')
            DEPENDABOT_SUMMARY=$(echo "$alert" | jq -r '.security_advisory.summary')
            
            cat >> "$OUTPUT_DIR/summary.html" << EOF
                <div class="alert-item $DEPENDABOT_SEVERITY">
                    <div class="alert-title">
                        $DEPENDABOT_PACKAGE
                        <span class="severity-badge severity-$DEPENDABOT_SEVERITY">$DEPENDABOT_SEVERITY</span>
                    </div>
                    <div class="alert-description">$DEPENDABOT_SUMMARY</div>
                </div>
EOF
        done
    else
    ...
    cat >> "$OUTPUT_DIR/summary.html" << EOF
        ...
        <div class="footer">
            <p>Generated by GitHub Security Reporter v$SCRIPT_VERSION</p>
...
EOF
    ...
    log_success "HTML report generated: $OUTPUT_DIR/summary.html"
else
    log_info "Using custom template or Node.js for report generation"
fi

log_success "Security analysis complete!"
log_info "Reports available in: $OUTPUT_DIR/"
log_info "  - summary.html: Interactive HTML dashboard"
log_info "  - data.json: Complete security data"
if [ -f "$OUTPUT_DIR/latest.sarif" ]; then
    log_info "  - latest.sarif: SARIF format analysis"
fi

echo ""
log_info "Total findings:"
log_info "  Code Scanning: $CODE_SCANNING_COUNT alerts"
log_info "  Secret Scanning: $SECRET_SCANNING_COUNT alerts"
log_info "  Dependabot: $DEPENDABOT_COUNT alerts"
